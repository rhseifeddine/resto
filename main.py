...
class RestaurantApp(MDApp):
    ...
    def fetch_admin_stats(self, show_notification=True):
        ...
        def on_success(req, result):
            if result.get('status') == 'success':
                ...
                self.admin_screen.ids.lbl_ventes_payees.text = format_dz(data.get('ventes_payees', 0))
                self.admin_screen.ids.lbl_dettes_ventes.text = format_dz(data.get('dettes_ventes', 0))
    ...
    def download_report_mobile(self, report_type):
        if not getattr(self, 'admin_start_date', None) or not getattr(self, 'admin_end_date', None):
            ...
            self.admin_end_date = self.admin_start_date
        value_text = '0 DA'
        if report_type == 'net_caisse':
            value_text = self.admin_screen.ids.lbl_caisse.text
        elif report_type == 'ventes_payees':
            value_text = self.admin_screen.ids.lbl_ventes_payees.text
        elif report_type == 'dettes_ventes':
            value_text = self.admin_screen.ids.lbl_dettes_ventes.text
        if value_text.strip() == '0 DA':
            self.notify('Aucune transaction trouvée (0 DA). Téléchargement annulé.', 'warning')
            return
        report_names = {'net_caisse': 'TOTAL EN CAISSE', 'ventes_payees': 'Ventes Payées', 'dettes_ventes': 'Dettes Ventes'}
        rep_name = report_names.get(report_type, 'Rapport')
        dialog_text = f"Voulez-vous vraiment télécharger le rapport '{rep_name}' ?\n(Du {self.admin_start_date} au {self.admin_end_date})"
        if hasattr(self, 'download_dialog') and getattr(self, 'download_dialog', None):
            self.download_dialog.dismiss()
        from kivymd.uix.dialog import MDDialog
        from kivymd.uix.button import MDFlatButton, MDRaisedButton
        self.download_dialog = MDDialog(title='Confirmation de téléchargement', text=dialog_text, buttons=[MDFlatButton(text='ANNULER', theme_text_color='Error', on_release=lambda x: self.download_dialog.dismiss()), MDRaisedButton(text='TÉLÉCHARGER', md_bg_color=(0, 0.6, 0.2, 1), on_release=lambda x: self._execute_direct_download(report_type))])
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if 'application/json' in content_type:
                    try:
                        data = response.json()
                        self._show_download_error(data.get('message', 'Aucune transaction à télécharger.'))
                    except:
                ...
                with open(filepath, 'wb') as f:
                    for chunk in response.iter_content(chunk_size=8192):
                        f.write(chunk)
                self._show_download_success(filepath)
            else:
                self._show_download_error(f'Erreur serveur: {response.status_code}')
    ...
    def _execute_direct_download(self, report_type):
        if hasattr(self, 'download_dialog') and getattr(self, 'download_dialog', None):
            self.download_dialog.dismiss()
        self.notify('Téléchargement en cours...', 'info')
        import threading
        threading.Thread(target=self._download_worker, args=(report_type,), daemon=True).start()
    ...
    def _show_download_error(self, err_msg):
        self.notify(f'Échec du téléchargement: {err_msg}', 'error')
